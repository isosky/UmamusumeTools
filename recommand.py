...
temp = get_recommand_uma_list(target=target, borrow=uma_borrow, is_father=False, mua_min_factor=2, mua_min_blue=4, is_bigmatch=True, result_length=5)
if uma_borrow == '':
    if temp:
        s = 1
        for i in temp:
            ...
            log.info(f" 指定各因子命中详情[父，爷，奶，母，公，婆]：{i['couple_factor_match']}")
            s += 1
    else:
